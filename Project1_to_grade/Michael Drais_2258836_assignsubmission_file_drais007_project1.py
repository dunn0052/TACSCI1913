...
class Random:
	...
	def next(self):
		#print("Random next")
		new = (16807 * self.__value) % (2147483648 -1)
		self.__value = new
		return new
	def choose(self, max):
		#print ("Ramdom Choose")
...
class Mumble:
	def __init__(self, seed, choosers):
		#print("mumble initializing")
		self.__rng = Random(seed)
		self.__choosers = choosers
	def make(self, max):
		#print("mumble make function")
		word = ""
		index = 2
		if(max <= 0):
			raise RuntimeError
			...
			for x in range(max):
				if(x == 0):
				#	print("first letter case")
					index = 2
					value = self.__rng.choose(self.__choosers[" "][0])
				#	print("value = " + str(value))
					while(True):
						value -= self.__choosers[" "][index]
					#	print("value = " + str(value))
						if(value <= 0):
							break
						else:
							index += 2
				#	print("letter = " + choosers[" "][index-1])		
					word += self.__choosers[" "][index-1]			
				else:
				#	print("** other letters case")
				#	print("current word: " + word)			
					value = self.__rng.choose(self.__choosers[word[-1]][0])
				#	print("value = " + str(value))
					index = 2;
					while(True):
						value -= self.__choosers[word[-1]][index]
					#	print("value = " + str(value))
						if(value <= 0):
							break
						else:
							index += 2
					if(self.__choosers[word[-1]][index-1] == " "):
					#	print("end of string")
						return word
					else:			
						word += self.__choosers[word[-1]][index-1]
				#	print("letter = " + choosers[word[-1]][index-1])					
		return word				
				
	def test(self, count, min, max):
		#print("in mumble test")
		if(1 <= min and min <= max):
			#print ("no Errors")
			while(count > 0):
				word = self.make(max)
				if (len(word) >= min):
