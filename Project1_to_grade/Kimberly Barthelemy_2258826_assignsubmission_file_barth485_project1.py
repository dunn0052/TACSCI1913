...
class Mumble:
    ...
    def make(self, max):
        if (max<=0):
            ...
            self.word=''
            x=' '
            while (len(self.word)<max):
                restofword=self.choosers[x]
                finisher=self.number.choose(restofword[0])
                third=0
                while (finisher>0):
                    third+=2
                    subtract=restofword[third]
                    finisher-=subtract
                x=restofword[third-1]
                if (x == ' '):
                    break
                else:
