class Random:
    def __init__(self, seed): # seed is the first integer of the series
        self.seed = seed
        self.sevenfive = 7**5
        self.twopower = ((2**31) - 1)

# gets next number in the series from park-miller algorithm
    def next(self):
        self.seed = ((self.sevenfive) * self.seed) % (self.twopower)  # park miller algorithm
        return self.seed

# generates a random integer that is greater than or equal to 0, but less than max.
    def choose(self, max):
        return self.next() % max

class Mumble:
    def __init__(self, seed, choosers):
        self.seed = seed # allow seed
        self.choosers = choosers # allow chooser to be used
        self.random = Random(seed) # allow access to Random class
    ...
    def make(self, max):
        ...
        if max<=0:
            ...
            for x in range(max):
                # get one char
                if x==0:
                    # start with blank since last element is ' '
                    choose_max = self.choosers[word[len(word)-1]][0]
                else:
                    # chooser last letter of a word chooser tuple total - second letter of chooser total
                    choose_max = self.choosers[word[len(word)-1]][0] - self.choosers[word[len(word)-1]][2]
                number = self.random.choose(choose_max)
                while (number > 0): # limit set in theory section so it stops at the correct character
                    count += 2
                    if not(x!=0 and count==2):
                        number -= self.choosers[word[len(word)-1]][count]
                word += self.choosers[word[-1:]][count-1] # gets the character based on the previous one, and where the loop stopped
                print(word)
                count = 0
            return word
    ...
    def test(self, count, min, max):
        # print count random words, one per line, by calling make
        if 1>min or min>max: # checks min and max, to make sure they are mins and maxs
            raise RuntimeError('max must be greater than 0')
        else: # loop through make 'count' times
            for x in range(count):
                local_max = self.random.choose(max+1-min) # random number within length of range of min to max
                local_max += min # sets the local max into the true range
                print (self.make(local_max)) # creates a word of a length from mix to max
...
mumbler = Mumble(101, choosers) # creates an instance mumbler of Mumble, with 101 as a seed, and predefined choosers
