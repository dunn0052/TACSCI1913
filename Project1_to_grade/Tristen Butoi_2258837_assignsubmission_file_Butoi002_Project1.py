RandConst = 7**5 #used in the Park-Miller algorithm
RandMod = 2**31 - 1 #used for % in Park-Miller algorithm

class Random:
	def __init__(self, seed):
		self.__previous = seed #sets up a number used for next
		
	def next(self):
		self.__previous = (RandConst * self.__previous) % RandMod
...
class Mumble:
	...
	def getLetter(self, Range, letter): #gets a single letter 
		evenIndex = 2
		while Range > 0:
			Range = Range - self.__choosers[letter][evenIndex]
			if Range <= 0:
				return self.__choosers[letter][evenIndex - 1]
			evenIndex += 2
				
	def make(self, Max):#uses getLetter to string together a word and return it
		if Max <= 0:
			raise ValueError("A word cannot be less than or equal to 0 characters")
		else:
			R = self.__r.choose(self.__choosers[' '][0])
			self.L = ' ' #this s a letter attribute that changes	
			self.L = self.getLetter(R, self.L)
			self.word = self.L #this is an attribute so it does not get deleted at end of scope
			Max -= 1
			
			while Max > 0:
				R = self.__r.choose(self.__choosers[self.word[-1]][0])
				self.L = self.getLetter(R, self. L)
				if self.L == ' ':
					break
				self.word += self.L
	...
	def test(self, count, Min, Max):
		if Min < 1 or Min > Max:
			raise ValueError("a minimum value must be between 1 and max inclusively")
		else:
			while count > 0:
				word = self.make(Max)
				if len(word) >= Min:
...
m = Mumble(101, choosers)
print(m.test(50, 5, 10))
