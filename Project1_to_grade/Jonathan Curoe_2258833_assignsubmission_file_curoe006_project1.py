class Random:
  def __init__(self, seed):
    self.value = (16807 * seed) % (2147483648 - 1)

  def next(self):
    self.value = (16807 * self.value) % (2147483648 - 1)
    return self.value

  def choose(self, max): #returns random number in range max
...
class Mumble:
  ...
  def make(self, max):
    ...
    thisChar = ' '
    word = ''
    i = 0 #iterator for char index in current word
    while i < max:
      rand = self.r.choose(choosers[thisChar][0]) #random in range of first of thisChar's chooser
      j = 0 #iterator for index in thisChar's chooser
      while rand > 0: #subtracts letter count of letter at index in thisChar's chooser until 0 or below
        j += 2
        rand -= choosers[thisChar][j]
      thisChar = choosers[thisChar][j - 1] #updates thisChar to character when rand = 0 or below
      if thisChar == ' ': #ends word at space
        break
      word += thisChar #otherwise adds character to word
      i += 1
    return word
  ...
  def test(self, count, min, max):
    ...
    while count > 0:
      word = self.make(max)
      if len(word) >= min: #omits words shorter than min length
        print(word)
        count -= 1
