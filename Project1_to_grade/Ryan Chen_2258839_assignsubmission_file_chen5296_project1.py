...
#
# SUMMARY: The class Mumble generates random strings that somewhat look like English
#           words. Mumble uses a random number generator created with the class
#           Random. An instance of Random generates pseudo-random integers
#           with the Park-Miller algorithm. Mumble then uses a 'chooser' to select
#           the letters in the generated word. For example, a version of Mumble
#           produced nonsense words like adrisater, apongom, and blarore.

#creates a pseudo-random number generator
class Random:
    #constants that are used to calculate a random number
    _BIG_NUM_1 = 7**5
    _BIG_NUM_2 = (2**31)-1

    def __init__(self, seed):
        self.__randomNumber = seed

    #generates the next random integer in the series, and returns it
    def next(self):
        self.__randomNumber = (self._BIG_NUM_1 * self.__randomNumber) % self._BIG_NUM_2
        return self.__randomNumber

    #generates a random integer n, such that 0 <= n < max
    def choose(self, max):
        return self.next() % max

#generates random words utilizing a number generator from Random
class Mumble:

    def __init__(self, seed, choosers):
        self.__randomGen = Random(seed)
        self.__choosers = choosers

    #makes a new random word that has at most 'max' letters
    def make(self, max):
        if max <= 0:
            raise RuntimeError
        else:
            ...
            for iteration in range(max):
                if key == ' ' and iteration > 0:
                    break
                else:
                    R = self.__randomGen.choose(self.__choosers[key][0])
                    for index, chooserNumber in enumerate(self.__choosers[key][1:]):
                        if index % 2 == 1: #guarantees we're at a chooser's letter count
                            R -= chooserNumber
                            if R <= 0:
                                key = self.__choosers[key][index]
                                word += key
                                break
            return word

    #print 'count' number of random words, one per line.
    def test(self, count, min, max):
        if not(1 <= min and min <= max):
            raise RuntimeError
        else:
            printedWords = 0
            while printedWords < count:
                word = self.make(max)
                if len(word) >= min + 1: #if len(word) < max, the last letter is always a space
                    print(word)
                    printedWords += 1
