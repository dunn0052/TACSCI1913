...
#from src.Project1.Choosers import choosers

# CSCI 1913 Project 1 Will Davies (davie304@umn.edu)
# Generates random English looking words
class Mumble:
    ...
    # Generates one randomized word with length less than max
    def make(self, max):
        if max <= 0:
            raise RuntimeError("Word length must be greater than 0.")

        # Generate the first char assuming last char was ' ', ' ' isn't actually added to the word
        word = self.letter(' ')

        # Add letters until the word ends in a ' ' or meets the max length criteria
        while len(word) < max:
            le = self.letter(word[-1:])
            if le == ' ':
                return word
            else:
                word += le
    ...
    # Helper function that figures out the next most probable letter
    def letter(self, last_char):
        # Select the target tuple
        t = self.choosers[last_char]

        # Selects the sum for the specified char
        r = self.rand.choose(t[0])

        # Loop from left to right through the chars, as soon as r gets to <= 0, use that char
        for index in range(2, len(t), 2):
            r -= t[index]
            if r <= 0:
                return t[index - 1]

    # Testing function for generating lots of words
    def test(self, count, min, max):
        # Disallow invalid bounds
        if not (1 <= min <= max):
            raise RuntimeError("Invalid min and max word lengths")

        while count > 0:
            word = self.make(max)
            if len(word) >= min:  # Only words with length > min are printed, otherwise ignored
                count -= 1
                print(word)


# Generates pseudo-random numbers using Park-Miller series
class Random:

    # Constants so they aren't recomputed each loop
    s = pow(7, 5)
    t = pow(2, 31)
