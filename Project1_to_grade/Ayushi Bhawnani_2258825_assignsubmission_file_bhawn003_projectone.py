...
class Random:
    ...
    # Initialize an instance of Random so it generates the series of random integers that follow seed.
    ...
    #Generate the next random integer in the series, and return it.
        self.__R = (16807*self.__R)%(2147483647)
        return self.__R


    def choose(self, max):
    #Use next to generate a random integer that is greater than or equal to 0, but less than max.
    #You may assume that max is an integer greater than 0.
        n = self.next()
        if n >= max:
            return n % max
        return n
...
class Mumble:
    ...
    def make(self, max):
        ...
        while i < max:
            ...
            R = self.__N.choose(self.__choosers[c][0])

            j = 0
            while R > 0:
                j = j + 2
                R -= self.__choosers[c][j]


            c = self.__choosers[c][j-1]
            if c != ' ':
                s += c
            i += 1
        return s
