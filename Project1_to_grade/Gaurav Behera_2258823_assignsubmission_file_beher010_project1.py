...
class Random:
    ...
    def next(self):
        self.seed = (self.pm_min * self.seed) % (self.pm_max - 1)
        return self.seed
...
class Mumble:
    ...
    def make(self, max):
        if(max <= 0):
            ...
            start_tuple = self.choosers.get(' ')
            i = 0
            word = []
            for x in range(0, max):
                R = self.Rs.choose(start_tuple[i])
                while R > 0:
                    i += 2
                    R -= start_tuple[i]
                    if (R <= 0):
                        word.append(start_tuple[i-1])
                if (start_tuple[i-1] == ' '):
                    word.pop()
                    return_value = ''.join(word)
                    return return_value
                else:
                    next_tuple = self.choosers.get(start_tuple[i-1])
                    start_tuple = next_tuple
                    i = 0
            return ''.join(word)
